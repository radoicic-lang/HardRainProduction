...
export default function App() {
  ...
  // Interactive Scope Builder / Gemini AI Integration
  const [aiAssistantPrompt, setAiAssistantPrompt] = useState("");
  const [aiResponseText, setAiResponseText] = useState("");
  const [isGeneratingBrief, setIsGeneratingBrief] = useState(false);
  const [generationType, setGenerationType] = useState<"brief" | "concept" | "budget">("brief");
  ...
  // Call Gemini API server-side endpoint for interactive scope building
  const queryCreativeBriefAPI = async (type: "brief" | "concept" | "budget", customPrompt?: string) => {
    setIsGeneratingBrief(true);
    setGenerationType(type);

    let finalPrompt = customPrompt || aiAssistantPrompt;
    if (!finalPrompt.trim()) {
      if (type === "brief") {
        finalPrompt = "Draft a production brief for a luxury smartwatch narrative marketing campaign.";
      } else if (type === "concept") {
        finalPrompt = "Provide an abstract sci-fi dynamic visual treatment with anamorphic lens artifacts.";
      } else {
        finalPrompt = "Suggest a structured budget allocation guide for a $120k tech hardware commercial.";
      }
    }
    ...
    try {
      const response = await fetch("/api/gemini/generate", {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          projectType: selectedHeroItem.category,
          budgetGrade: selectedHeroItem.budgetGrade,
          context: "Platform of Executive Producer Scott Bernstein, Hard Rain Production, optimizing search footprint."
        })
      });

      const data = await response.json();
      setAiResponseText(data.text || "No output generated by the cinematic model.");
    } catch (e: any) {
      console.error(e);
      setAiResponseText(`Error connecting to creative model: ${e.message}. Utilizing simulation engine mode...`);
    } finally {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-zinc-200 pb-8">
          ...
          <div className="w-full md:w-auto space-y-4">
            ...
            <div className="relative w-full md:w-80">
              ...
              />
              <span className="absolute right-3.5 top-3 text-[10px] font-mono text-blue-600 flex items-center gap-1 pointer-events-none">
                <Sparkles className="w-3.5 h-3.5 text-blue-600 animate-pulse" /> AI ENGINE
              </span>
            </div>
          ...
          <div className="lg:col-span-7 bg-zinc-50 border border-zinc-200 rounded-3xl p-6 md:p-8 flex flex-col justify-between shadow-sm relative overflow-hidden">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => {
                    setAiAssistantPrompt("Draft a production brief for a luxury smartwatch narrative marketing campaign.");
                    queryCreativeBriefAPI("brief", "Draft a production brief for a luxury smartwatch narrative marketing campaign.");
                  }}
                  className="px-3 py-1 bg-white hover:bg-zinc-100 transition text-[10px] font-mono text-zinc-700 rounded-full border border-zinc-200 cursor-pointer shadow-sm"
                >
                  ⚡ Watch Brand Brief
                </button>
                <button
                  onClick={() => {
                    setAiAssistantPrompt("Provide an abstract sci-fi dynamic visual treatment with anamorphic lens artifacts.");
                    queryCreativeBriefAPI("concept", "Provide an abstract sci-fi dynamic visual treatment with anamorphic lens artifacts.");
                  }}
                  className="px-3 py-1 bg-white hover:bg-zinc-100 transition text-[10px] font-mono text-zinc-700 rounded-full border border-zinc-200 cursor-pointer shadow-sm"
                >
                  🎨 Sci-Fi Concept
                ...
                <button
                  onClick={() => {
                    setAiAssistantPrompt("Suggest a structured budget allocation guide for a $120k tech campaign on static volumes.");
                    queryCreativeBriefAPI("budget", "Suggest a structured budget allocation guide for a $120k tech campaign on static volumes.");
                  }}
                  className="px-3 py-1 bg-white hover:bg-zinc-100 transition text-[10px] font-mono text-zinc-700 rounded-full border border-zinc-200 cursor-pointer shadow-sm"
                >
                  📊 $120k Budget Grid
              ...
              <div className="space-y-2">
                <textarea
                  rows={2}
                  placeholder="Ask me: 'Prepare a 4-day shoot timeline for an Icelandic travel documentary on RED cameras'..."
                  value={aiAssistantPrompt}
                  onChange={(e) => setAiAssistantPrompt(e.target.value)}
                  className="w-full p-3.5 bg-white border border-zinc-200 rounded-2xl text-xs text-zinc-800 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-600/10 focus:border-blue-600 border-zinc-200 transition font-mono leading-relaxed resize-none shadow-sm"
                />

                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-mono text-zinc-400">SGE tags will auto-attach</span>
                  <button
                    disabled={isGeneratingBrief}
                    onClick={() => queryCreativeBriefAPI("brief")}
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 transition text-white text-xs font-mono font-medium rounded-full flex items-center gap-1.5 cursor-pointer shadow-md border border-blue-600"
                  >
                    {isGeneratingBrief ? "GENERATING..." : "BUILD CINEMATIC BLUEPRINT"} <Sparkles className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>

              {/* Output Display Panel */}
              <div className="border border-zinc-200 bg-white rounded-2xl p-5 overflow-y-auto max-h-72 min-h-36 font-sans text-xs shadow-sm">
                {isGeneratingBrief ? (
                  <div className="flex flex-col items-center justify-center py-12 space-y-3">
                    <div className="animate-spin h-5 w-5 border-2 border-blue-600 border-t-transparent rounded-full" />
                    <span className="font-mono text-[10px] text-zinc-500 uppercase tracking-widest animate-pulse">Consulting Creative Model...</span>
                  </div>
                ) : aiResponseText ? (
                  <div className="prose prose-xs leading-relaxed text-zinc-800 max-w-none font-mono">
                    <div className="flex items-center justify-between border-b border-zinc-100 pb-2 mb-3 text-[10px] text-zinc-400">
                      <span>TYPE: {generationType.toUpperCase()} OUTPUT</span>
                      <span className="text-blue-600 font-bold">READY FOR EXPORT</span>
                    </div>
                    
                    {/* Rendered simulated/live markdown lines cleanly */}
                    <div className="space-y-2 text-zinc-700 font-mono text-[11px] whitespace-pre-wrap leading-relaxed">
                      {aiResponseText}
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center py-12 text-zinc-400 font-mono text-center space-y-1.55">
                    <Compass className="w-6 h-6 stroke-1 text-zinc-300" />
                    <p className="text-[10px] uppercase tracking-wider text-zinc-400">BLUEPRINT CANVAS EMPTY</p>
                    <p className="text-[9.5px] text-zinc-400 max-w-xs font-sans leading-normal">Select any sample chip above or input a custom prompt message to generate structured models instantly.</p>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6 min-w-full font-mono text-xs text-zinc-600 pt-2">
              ...
              <div className="p-4 border border-zinc-200 rounded-xl space-y-2 bg-white shadow-sm">
                ...
                <p className="text-zinc-500 text-[11px] leading-relaxed">Rich semantic schema, programmatic /ai-agents.json manifest, searchable metadata logs.</p>
              </div>
            </div>
          </div>
      ...
      <footer className="py-12 px-4 border-t border-zinc-200 max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6 text-xs text-zinc-500 font-mono">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span>© 2027 Hard Rain Production. All rights reserved.</span>
          </div>
        </div>
        <div className="flex gap-4">
          ...
          <a href="/api/ai-agent-manifest" target="_blank" className="hover:text-blue-500 text-blue-600 font-bold flex items-center gap-1">
            <Cpu className="w-3 h-3" /> /ai-agents.json
          </a>
        </div>
      </footer>
